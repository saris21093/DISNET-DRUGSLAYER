...
source_id=int(("SELECT source_id from source where name = '%s'" % SOURCE)[0][0])
    ...
    if not drug_id in drug_pk_table: 
        ...
        if not (drug_id,DRUGBANK_RESOURCE_ID,DRUG_ENTITY_ID) in new_code_list:  
            ...
            if response:
                ...
                code=(db_code,DRUGBANK_RESOURCE_ID,DRUG_ENTITY_ID)
                has_code=(drug_id,db_code,DRUGBANK_RESOURCE_ID,DRUG_ENTITY_ID)
                chembl_cr=(drug_id,DRUGBANK_RESOURCE_ID,DRUG_ENTITY_ID)
