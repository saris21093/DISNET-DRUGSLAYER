...
# Get the de MeSH code and the UMLS code from has_code table, and create a diccionary with MeSH as key and Code as  value
...
# Get the drugs from the database in order to obtain the indication just for the approved ones, these data will be keep in a list        
drugs_table=get_list("select distinct drug_id from drug")
drugs_table=list(*zip(*drugs_table))

# Get the primary keys (pk) from phenotype_effect where source = 1 -chEMBL-
pe_table = get_list("select phenotype_id from phenotype_effect where source_id = 1")
pe_table=list(*zip(*pe_table))

# Get the drug_phenotype_effect table
drug_pe_pk_table = get_list("select phenotype_id, drug_id, source_id from drug_phenotype_effect where source_id = 1")

# Get the primary keys (pk) from drug_phenotype_effect table
drug_pe_table = get_list("select * from drug_phenotype_effect where phenotype_type = 'INDICATION'")
...
for i in indications:
    ...
    if drug_id in drugs_table:
        if indication_mesh in mesh_umls_dic.keys():
            ...
            if not drug_indication_pk in drug_pe_pk_table:
                if not drug_indication_pk in new_drug_indication_list:
                    new_drug_indication_list.append(drug_indication_pk)
                    drug_indication_list.append(drug_indication)
                else:
                    index = new_drug_indication_list.index(drug_indication_pk)
                    if drug_indication_list[index][3] < max_phase:
                        drug_indication_list.pop(index)
...
cursor.executemany("insert into drug_phenotype_effect values(%s,%s,%s,%s,%s)",drug_indication_list)
n_ins_drug_indication = len(drug_indication_list)
